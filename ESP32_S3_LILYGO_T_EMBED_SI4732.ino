...
void setup()
{
  ...
  Serial.println("[BOOT] PIN_POWER_ON high");
  ...
  Serial.println("[BOOT] encoder pins set");
  ...
  Serial.println("[BOOT] tft fill black done");
  uint8_t* sp = (uint8_t*)spr.createSprite(320,170);
  Serial.printf("[BOOT] createSprite ptr=%p free_psram=%u free_heap=%u\n",
...
*/
  ...
  Serial.println("[BOOT] EEPROM begin done");
  ...
  Serial.println("[BOOT] encoder ISRs attached");

  rx.setI2CFastModeCustom(100000);

  Serial.println("[BOOT] probing Si4735 I2C ...");
  int16_t si4735Addr = rx.getDeviceI2CAddress(RESET_PIN); // Looks for the I2C bus address and set it.  Returns 0 if error
  Serial.printf("[BOOT] Si4735 I2C addr=0x%02X\n", si4735Addr);
...
{ // rotary encoder events
  ...
  if (encoderStatus)
    encoderCount = (encoderStatus == DIR_CW) ? -1 : 1; // reversed: CCW (left) increments
}
